...
Sub-pages use the `contribution-page` wrapper div. Talk pages additionally add the `talk-page` class. Back-links on sub-pages point back to anchors on `index.html` (e.g. `../#talks`, `../#contributions`).
...
**No `.html` extension in canonical URLs, `href` links, or `<a>` href attributes** — `.htaccess` handles extension-less serving on production.
